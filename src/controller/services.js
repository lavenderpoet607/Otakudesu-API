    ...
    getOngoing: async (req, res) => {
        ...
        try {
            ...
            element.find("ul > li").each((index, el) => {
                ...
                let total_episode = $(el).find(".epz").text();
                let updated_on = $(el).find(".newnime").text();
                let updated_day = $(el).find(".epztipe").text();
                let endpoint = $(el).find(".thumb > a").attr("href").replace(`${baseUrl}/anime/`, "").replace("/", "");
                ongoing.push({ title, thumb, total_episode, updated_on, updated_day, endpoint });
            });
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                element.find("ul > li").each((index, el) => {
                    ...
                    let total_episode = $(el).find(".epz").text();
                    let updated_on = $(el).find(".newnime").text();
                    let score = $(el).find(".epztipe").text().trim();
                    let endpoint = $(el).find(".thumb > a").attr("href").replace(`${baseUrl}/anime/`, "").replace("/", "");
                    completed.push({ title, thumb, total_episode, updated_on, score, endpoint });
                });
                return res.status(200).json({
                    status: true,
                    message: "success",
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                element.find("li").each((index, el) => {
                    ...
                    let thumb = $(el).find("img").attr("src");
                    let genres = $(el).find(".set > a").text().match(/[A-Z][a-z]+/g);
                    let status = $(el).find(".set").text().match("Ongoing") || $(el).find(".set").text().match("Completed");
                    let rating = $(el).find(".set").text().replace(/^\D+/g, '') || null;
                    let endpoint = $(el).find("h2 > a").attr("href").replace(`${baseUrl}/anime/`, "").replace("/", "");
                    search.push({ title, thumb, genres, status, rating, endpoint });
                });
                return res.status(200).json({
                    status: true,
                    message: "success",
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                element.find(".jdlbar").each((index, el) => {
                    let title = $(el).find("a").text() || null;
                    let endpoint = $(el).find("a").attr("href").replace(`${baseUrl}/anime/`, "");
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                episodeElement.find("li").each((index, el) => {
                    ...
                    episode_list.push({ episode_title, episode_endpoint, episode_date });
                });
                return res.status(200).json({
                    status: true,
                    message: "success",
    ...
    getAnimeEpisode: async (req, res) => {
        ...
        try {
            ...
            $("#selectcog > option").each((index, el) => {
                ...
                obj.list_episode.push({ list_episode_title, list_episode_endpoint });
            });
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                $('.genres').find("a").each((index, el) => {
                    let genre = $(el).text();
                    let endpoint = $(el).attr('href').replace("/genres/", "").replace("/", "");
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                $('.col-anime-con').each((index, el) => {
                    let title = $(el).find(".col-anime-title > a").text();
                    let link = $(el).find(".col-anime-title > a").attr("href").replace(`${baseUrl}/anime/`, "");
                    let studio = $(el).find(".col-anime-studio").text();
                    let episode = $(el).find(".col-anime-eps").text();
                    let rating = $(el).find(".col-anime-rating").text() || null;
                    let thumb = $(el).find(".col-anime-cover > img").attr("src");
                    let season = $(el).find(".col-anime-date").text();
                    let sinopsis = $(el).find(".col-synopsis").text();
                    let genre = $(el).find(".col-anime-genre").text().trim().split(",");
                    genreAnime.push({ title, link, studio, episode, rating, thumb, genre, sinopsis });
                });
                return res.status(200).json({
                    status: true,
                    message: "success",
